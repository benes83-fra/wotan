...
heston_objective_fn :: proc(x: []f64, user_data: rawptr) -> f64 {
	...
	rmse := 0.0
	for point in ctx.market_data {
		// ✅ FIX 1: Increase integration points to 200 for a smoother gradient
		model_price := heston_price(ctx.S, point.strike, point.expiry, ctx.r, params, .Call, 200)

		// ✅ FIX 2: Softer penalty to prevent hard cliffs in the loss landscape
...
calibrate_heston :: proc(
	...
	for iter in 0 ..< max_iter {
		...
		x[4] = math.max(-0.99, math.min(0.99, x[4])) // rho
	}
...
heston_characteristic_function :: proc(
	...
	return cmath_exp(C + D * complex(params.v0, 0.0) + iu * cmath_log(complex(S, 0.0)))
}
