...
volatility_forecasting_test :: proc(allocator: mem.Allocator) {
	...
	fmt.printf(
		"Training MLP (Flat Input: %d, Hidden: %d, Seq: %d) for %d epochs...\n",
		flat_input,
		hidden_size,
		seq_len,
		epochs,
	...
	forecaster := ml_fin.volatility_forecaster_new(input_size, hidden_size, seq_len, allocator)
	...
	for s in 0 ..< num_samples {
		...
		for t in 0 ..< seq_len {
			offset := s * seq_len * input_size + t * input_size
	...
	for epoch in 0 ..< epochs {
		...
		for b in 0 ..< num_samples / batch_size {
			...
			x_batch_data := l.matrix_new(f64, batch_size, flat_input, allocator)
			copy(
				x_batch_data.data,
			...
			defer t.tensor_free_graph(preds)

			loss := t.tensor_mse_loss(preds, y_batch)
			defer t.tensor_free_graph(loss)
...
volatility_forecaster_add_to_optimizer :: proc(
	...
	nn.adam_add_param(opt, model.fc1_w)
	nn.adam_add_param(opt, model.fc1_b)
	nn.adam_add_param(opt, model.fc2_w)
	nn.adam_add_param(opt, model.fc2_b)
	nn.adam_add_param(opt, model.fc3_w)
	nn.adam_add_param(opt, model.fc3_b)
}
